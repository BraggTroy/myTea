...
ES.ui.popup = function(config) {
	...
	config = ES.util.merge({
		title: ES.msg.get('default_title2'),
...
ES.ui.radio = function(config) {
	...
	if (config.data) {
		ES.each(config.data, function (index, item) {
			radioInput += '<div class="radio-item"><label class="'+item.cls+'">' + item.display + '</label><input type="radio" name="' + ret.el.attr('name') + '" id="' + item.value + '"/></div>'
		})
	}
	var input = ES.get('#' + ret.id)

	ES.get('#' + ret.id).after('<div class="radio-group" name="' + ret.el.attr('name') + '">' + radioInput + '</div>')
	ES.get('#' + ret.id).siblings('.radio-group').find('.radio-item').on('click', function () {
...
ES.ui.input_filter = function(config) {
	...
	ret.input_listener = function(evt) {
	...
	if(window.attachEvent && document.getElementById(ret.id).onpropertychange){
		document.getElementById(ret.id).onpropertychange=ret.input_listener
	}else{
		ES.get('#' + ret.id).on('input',ret.input_listener)
		ES.get('#' + ret.id).on('click',ret.input_listener)
	}
...
ES.ui.input_form = function(config) {
	var es_input_form = function(conf) {
		...
		$.each(items_dom, function(i, v) {
			...
			var input
			if (conf.items[item_index].type == 'multi_select') {
		...
		this.init = function() {
			...
			for(var i = 0; i < this.headsLen; i++) {
				if(i != this.headsLen - 1) {
					thList.push('<th class="'+(this.items[i].cls=='import-export-toggle'?'title-toggle':'')+'">' + this.heads[i] + '</th>')
				} else {
...
ES.ui.show_container_img = function (target,imagePaths,closeMethod) {
	...
	for (var i = 0; i < imageLen; i++) {
		liHtml +=
			...
			"	<div class='pic-div'>" +
			"		<canvas></canvas>" +
			"		<a class='narrow'><i class='fa fa-search-minus' aria-hidden='true'></i></a>" +
			"		<a class='save'><i class='fa fa-floppy-o' aria-hidden='true'></i></a>" +
			"		<a class='rotate'><i class='fa fa fa-repeat' aria-hidden='true'></i></a>" +
			"		<a class='enlarge'><i class='fa fa-search-plus' aria-hidden='true'></i></a>" +
			"	</div>" +
			"</li>"
...
ES.ui.tableGrid = function (config) {
	var grid = function () {
		var defaultConfig = {
			...
			onlyCls = "-table-grid-" + new  Date().getTime(),
			gridClass = config.el + onlyCls;
		var isEdit = config.enableEdit || defaultConfig.enableEdit,
			...
			isFreezeColumn = config.freezeColumn || defaultConfig.freezeColumn;

		if(tableEl.length <= 0 ){
			return;
		}
		tableEl.addClass(gridClass);
		tableEl = $("."+gridClass);
		//当全局编辑是返回所有数据
		...
		var edit = function (config) {
			var isSingleRowModel  = config.isSingleRowModel || false,
				el = tableEl,
				tr = el.find("tbody tr"),
				...
				if(v.type == "filter"){
					...
					if(ES.tableGridEven[v.eventName]){
						...
						ES.get("#"+v.el).unbind(type).bind(type,function () {
							...
							if(isSingleRowModel){
								relatedFieldId = itemId[relatedFieldName];
							}
			...
			var editEvenRow = function (index) {
				...
				ES.each(th,function (_,v) {
					...
					if(type == "select"){
						...
						html += "<span class='input'><input type='text' id='"+id+"' class='number' value='"+displayValue+"'/></span>";
					}else if(type == "date"){
						html += "<span class='input'><input type='text' id='"+id+"' class='date' value='"+displayValue+"'/></span>";
					}else if(type == "datetime"){
						html += "<span class='input'><input type='text' id='"+id+"' class='datetime' value='"+displayValue+"'/></span>";
					}else{
						html += "<span class='input'><input type='text' id='"+id+"' value='"+displayValue+"' data-value='"+value+"' class='needEmpty'/></span>";
					}
					editTd.html(html);
					data.el = id;
					data.type = type;
			...
			if(isSingleRowModel){
				...
				var evenRealize = function (el) {
					...
					var freezeEL = undefined;
					if(isFreezeColumn && tableEl.find("th.freezeColumn").length > 0){
						var freeze = ES.get(".js-column-freeze"+onlyCls);
						ES.each(freeze,function (_,v) {
							if(ES.get(v).find("." + config.enabledEl).length > 0){
								freezeEL = ES.get(v);
								return false;
							}
						})
					...
					if(config.enabledEven){
						config.enabledEven({
							el: el,
							freezeEL: freezeEL.find("tbody tr").eq(index)
						});
					}
					if(isFocus){
					...
					var validate = function () {
						...
						ES.each(gridDataList[index],function (_,v) {
							...
							if(validate.split(",").indexOf("empty") >= 0 && value == ""){
								errorMsg = ES.msg.get("non_empty");
								ES.get("#"+v.el).attr("placeholder",errorMsg).addClass("error");
								result = false;
							}else if(validate.split(",").indexOf("lessDate") >= 0 && new Date().getTime() < new Date(value.replace(/-/g,"/")).getTime()){
					...
					if(freezeEL){
						saveEl = freezeEL.find("tbody tr").eq(index).find("."+rowModelSaveEl);
					}
					saveEl.unbind("click").bind("click",function () {
						var data = {},
							tr = ES.get(this).parents("tr.edit"),
							saveIndex =  ES.get(this).parents("tr").index();
						if(!validate()){
							return;
						}
						if(freezeEL){
							tr = tableEl.find("tbody tr").eq(saveIndex);
						}
						var oldDate = {};
						...
						returnData.errorEven = function () {
							ES.each(allTh,function (_,v) {
								...
								if(oldDate[options.name + "-display"]){
									...
									value = oldDate[options.name];
									td.find("span.view").html(oldDate[options.name]);
									td.find("span.input input").val(value);
								}
							})
						};
						returnData.freezeEL = freezeEL.find("tbody tr").eq(index);
					...
					if(freezeEL){
						cancelEl = freezeEL.find("tbody tr").eq(index).find("."+rowModelCancelEl);
					}
					cancelEl.unbind("click").bind("click",function () {
						var tr = ES.get(this).parents("tr.edit"),
							cancelIndex =  ES.get(this).parents("tr").index();
						tr.find("input").removeClass("error");
						if(freezeEL){
							tr = tableEl.find("tbody tr").eq(cancelIndex);
						}
				...
				if(enabledPattern == "dblclick"){
					//双击TR启动编辑模式
					tr.unbind("dblclick").bind("dblclick",function (e) {
						evenRealize(this);
						e.preventDefault()
					});
					...
					var openEl = tableEl;
					if(isFreezeColumn && tableEl.find("th.freezeColumn").length > 0){
						openEl = ES.get(".js-column-freeze" + onlyCls);
					}

					openEl.find("." + config.enabledEl).unbind("click").bind("click",function (e) {
						var trEl = ES.get(this).parents("tr"),
							index = trEl.index();
						if(isFreezeColumn && tableEl.find("th.freezeColumn").length > 0){
							trEl = tableEl.find("tbody tr").eq(index);
						}
						evenRealize(trEl);
						e.preventDefault()
					});
		...
		var freezeHead = function () {
			...
			var fixHeadMethod = function () {
				var headHeight = tableEl.prev(".js-fix-head").innerHeight(),
				...
				if(top >= 0){
					top = 0;
				}else if(((-top) + headHeight) >= tableHeight){
					top = -(tableHeight - headHeight);
				}
				...
				if(isFreezeColumn){
					tableEl.nextAll("table.js-fix-head-freeze"+onlyCls).css({
						top: -top
					});
				}
			};
		...
		var assistBar = function () {
			var html = "<div class='js-assist-bar assist-bar'>" +
					...
					"</ul>"+
					"</div>",
				originalData,originalColumnFreezeLeft,originalColumnFreezeRight,columnFreezeLeft,columnFreezeRight;
			if(ES.get("div.js-assist-bar").length <= 0){
				ES.get("body").append(html);
			}
			//数据信息
			var viewData = function (index,groupId) {
				var row = tableEl.find("tbody tr"),
					returnData = [];
				if(isFreezeColumn){
					columnFreezeLeft = [];
					columnFreezeRight = [];
				}
				//考虑优化问题，当数据大于500条数据不支持
				if(row.length > 500){
					return;
				}
				ES.each(row,function (_,v) {
					if(ES.get(v).hasClass("groupBy")){
						return true;
					}
					var data = {},
						td = ES.get(v).find("td").eq(index),
						name = td.text(),
						groupName = "",
						trIndex = ES.get(v).index();
					if(groupId){
						groupName = ES.get(v).find("td").eq(groupId).text();
					}
					if(td.hasClass("input")){
						name = td.find("span.view").text();
						if(groupId){
							groupName = ES.get(v).find("td").eq(groupId).find("span.view").text();
						}
					}
					if(groupName != ""){
					...
					data.name = name;
					data.index = trIndex;
					data.html = ES.get(v)[0].outerHTML;
					returnData.push(data);
					if(isFreezeColumn && ES.get(".js-column-freeze-right"+onlyCls).length > 0){
						var columnRightData = {};
						columnRightData.name = name;
						columnRightData.index = trIndex;
						if(groupName != ""){
							columnRightData.groupName = groupName;
						}
						columnRightData.html = ES.get(".js-column-freeze-right"+onlyCls).find("tbody tr").eq(trIndex)[0].outerHTML;
						columnFreezeRight.push(columnRightData);
					}
					if(isFreezeColumn && ES.get(".js-column-freeze-left"+onlyCls).length > 0){
						var columnLeftData = {};
						columnLeftData.name = name;
						columnLeftData.index = trIndex;
						if(groupName != ""){
							columnLeftData.groupName = groupName;
						}
						columnLeftData.html = ES.get(".js-column-freeze-left"+onlyCls).find("tbody tr").eq(trIndex)[0].outerHTML;
						columnFreezeLeft.push(columnLeftData);
					}
				});
				if(!originalData){
					originalData = returnData;
				}
				if(!originalColumnFreezeLeft){
					originalColumnFreezeLeft = columnFreezeLeft;
				}
				if(!originalColumnFreezeRight){
					originalColumnFreezeRight = columnFreezeRight;
				}
				return returnData;
			};
			// 数据排序
			var viewSortData = function (conf) {
				var config = conf || {},
					data = config.data || [],
					sortType = config.type || "asc",
				...
				data.sort(function (a,b) {
					var sortNameA = groupId ? a.groupName : a.name,
						sortNameB = groupId ? b.groupName : b.name;
			...
			//数据分组
			var viewGroupData = function (conf) {
				var config = conf || {},
					data = config.data || [],
					sortType = config.type || "",
					groupId = config.groupId;
				if(data.length <= 0){
					return;
				}
				...
				ES.each(data,function (_,v) {
					if(groupId){
						if(groupViewData[v.groupName]){
							groupViewData[v.groupName].data.push(v);
							return true;
						}
						groupViewData[v.groupName] = {};
						groupViewData[v.groupName].name = v.groupName;
						groupViewData[v.groupName].data = [v];
					}else{
						if(groupViewData[v.name]){
							groupViewData[v.name].data.push(v);
							return true;
						}
						groupViewData[v.name] = {};
						groupViewData[v.name].name = v.name;
						groupViewData[v.name].data = [v];
					}
				});
				if(sortType != ""){
					ES.each(groupViewData,function (_,v) {
						var sData = viewSortData({
							data: v.data,
							type: sortType
						});
						v.data = sData;
			...
			var viewDataForHtml = function (conf) {
				var config = conf || {},
					data = config.data || [],
				...
				if(type == "view"){
					...
					ES.each(data,function (_,v) {
						var len = tableEl.find("thead th").length,
							groupHtml = "<tr class='groupBy'><td colspan='"+len+"' class='title'><b>"+v.name+"("+v.data.length+")</b></td></tr>";
						if(isFreeze){
							len = ES.get("." + config.el).find("thead th").length;
							groupHtml = "<tr class='groupBy'><td colspan='"+len+"' class='title'>&nbsp;</td></tr>";
						}
						ES.each(v.data,function (k,vl) {
							groupHtml += vl.html;
						});
						html += groupHtml;
					});
				...
				if(isFreeze){
					ES.get("." + config.el).find("tbody").html(html);
				}else{
			...
			var setAuxiliaryBar = function () {
				...
				ES.each(th,function (_,v) {
					...
					var html = '<span class="operation-list"><i class="fa fa-sort sort js-sort" aria-hidden="true"></i><span class="assist-operation js-show-assist" data-value="'+_+'"><i class="fa fa-bars" aria-hidden="true"></i></span></span>';
					ES.get(v).addClass("auxiliary").append(html);
					if(isFreezeHead){
						tableEl.prev("table.js-fix-head").find("thead th").eq(ES.get(v).index()).addClass("auxiliary").append(html);
					}
				});
				ES.get(window).resize();
			};
			...
			var bindEven = function () {
				...
				tableEl.find("thead th.auxiliary").hover(function () {
					ES.get(this).find("span.js-show-assist").addClass("active");
					// showAssistBar(this);
				},function () {
					if(ES.get("div.js-assist-bar").css("visibility") == "hidden"){
						ES.get(this).find("span.js-show-assist").removeClass("active");
					}
				});
				if(isFreezeHead){
					tableEl.prev("table.js-fix-head").find("thead th.auxiliary").hover(function () {
						ES.get(this).find("span.js-show-assist").addClass("active");
					...
					tableEl.prev("table.js-fix-head").find("span.js-show-assist").unbind("click").bind("click",function () {
						tableEl.prev("table.js-fix-head").find("span.js-show-assist").removeClass("active");
						ES.get(this).addClass("active");
						var el = ES.get(this).parents("th.auxiliary");
						showAssistBar(el);
					});
				}
				tableEl.find("span.js-show-assist").unbind("click").bind("click",function () {
					tableEl.prev.find("span.js-show-assist").removeClass("active");
					ES.get(this).addClass("active");
					var el = ES.get(this).parents("th.auxiliary");
					showAssistBar(el);
				});
				ES.get("div.js-assist-bar li.js-asc").unbind("click").bind("click",function () {
					var id = ES.get("div.js-assist-bar").data("value"),
						data = viewSortData({
							data: viewData(id),
							type: "asc"
						}),
						viewType = "view",
						groupId = ES.get("div.js-assist-bar").data("group") || -1;
					if(tableEl.find("tr.groupBy").length > 0){
						data = viewSortData({
							data: viewData(id,groupId),
							groupId: groupId
						});
						var groupData = viewGroupData({
							data: data,
							type: "asc",
							groupId: groupId
						});
					...
					if(columnFreezeRight && columnFreezeRight.length > 0){
						var columnRightData = viewGroupData({
							data: columnFreezeRight,
							type: "asc",
							groupId: groupId
						});
						viewDataForHtml({
							data: columnRightData,
							type: viewType,
							isFreeze: true,
							el: "js-column-freeze-right" + onlyCls
						});
					}
					if(columnFreezeLeft && columnFreezeLeft.length > 0){
						var columnLetfData = viewGroupData({
							data: columnFreezeLeft,
							type: "asc",
							groupId: groupId
						});
						viewDataForHtml({
					...
					tableEl.prev("table.js-fix-head").find("span.js-show-assist").removeClass("active");
					tableEl.find("span.js-show-assist").removeClass("active");
				});
				...
				ES.get("div.js-assist-bar li.js-group").unbind("click").bind("click",function () {
					...
					ES.get("div.js-assist-bar").data("group",id);
					viewDataForHtml({
						data: data,
						type: "group"
					});
				...
				ES.get("div.js-assist-bar li.js-unGroup").unbind("click").bind("click",function () {
					...
					ES.get("div.js-assist-bar").data("group",-1);
					viewDataForHtml({
						data: originalData
					});
					originalData = undefined;
					if(isFreezeColumn && originalColumnFreezeLeft.length > 0){
						viewDataForHtml({
							data: originalColumnFreezeLeft,
							isFreeze: true,
							el: "js-column-freeze-left" + onlyCls
						});
						originalColumnFreezeLeft = undefined;
					}
					if(isFreezeColumn && originalColumnFreezeRight.length > 0){
						viewDataForHtml({
							data: originalColumnFreezeRight,
							isFreeze: true,
							el: "js-column-freeze-right" + onlyCls
						});
						originalColumnFreezeRight = undefined;
					}
		...
		var freezeColumn = function () {
			var freezeColumnItems = tableEl.find("thead th.freezeColumn"),
				freezeColumnData = tableEl.find("tbody tr"),
				leftColumnHead = "",leftColumnBody = "",
				rightColumnHead = "",rightColumnBody = "",
				leftColumnIndex = [],rightColumnIndex = [];
			// 因为需要copy列大于500条数据不支持
			if(freezeColumnData.length > 500){
				return;
			}
			ES.each(freezeColumnItems,function (_,v) {
				var el = ES.get(v),
					index = el.index(),
					alignment = el.data("value"),
					width = el.outerWidth(),
					value = el.html();
				if(alignment.toUpperCase() == "LEFT"){
					leftColumnHead += "<th style='width: "+width+"px;'>"+value+"</th>";
					leftColumnIndex.push(index);
				}else if(alignment.toUpperCase() == "RIGHT"){
					rightColumnHead += "<th style='width: "+width+"px;'>"+value+"</th>";
					rightColumnIndex.push(index);
				}
			});

			ES.each(freezeColumnData,function (_,v) {
				var tr = ES.get(v),
					td = tr.find("td"),
					trCls = tr.attr("class") || "",
					trDataValue = tr.data("value") || "",
					rightNewTr,leftNewTr;
				if(rightColumnIndex.length > 0 ){
					rightNewTr = "<tr data-value='"+trDataValue+"' class='"+trCls+"'>";
					ES.each(rightColumnIndex,function (k,vl) {
						rightNewTr += td.eq(vl)[0].outerHTML;
					});
					rightNewTr += "</tr>";
					rightColumnBody += rightNewTr;
				}
				if(leftColumnIndex.length > 0 ){
					leftNewTr = "<tr data-value='"+trDataValue+"' class='"+trCls+"'>";
					ES.each(leftColumnIndex,function (k,vl) {
						leftNewTr += td.eq(vl)[0].outerHTML;
					});
					leftNewTr += "</tr>";
					leftColumnBody += leftNewTr;
				}
			});
			if(rightColumnIndex.length > 0 ){
				var rightColumnHtml = "<table class='js-column-freeze-right"+onlyCls+" js-column-freeze"+onlyCls+" alignment-right column-freeze' data-value='right'><thead>"+rightColumnHead+"</thead><tbody>"+rightColumnBody+"</tbody></table>";
				tableEl.after(rightColumnHtml);
				var rightEl = tableEl.nextAll("table.alignment-right.column-freeze"),
					freezeRightWidth = rightEl.width(),
					fixRightColumnHtml = "<table class='js-fix-head-freeze"+onlyCls+" fix-head-freeze-right fix-head-freeze' style='width: "+freezeRightWidth+"px; z-index: 1;'><thead>"+rightColumnHead+"</thead></table>";
				rightEl.before(fixRightColumnHtml);
			}
			if(leftColumnIndex.length > 0 ){
				var leftColumnHtml = "<table class='js-column-freeze-left"+onlyCls+" js-column-freeze"+onlyCls+" alignment-left column-freeze' data-value='left'><thead>"+leftColumnHead+"</thead><tbody>"+leftColumnBody+"</tbody></table>";
				tableEl.after(leftColumnHtml);
				var leftEl = tableEl.nextAll("table.alignment-left.column-freeze"),
					freezeLeftWidth = leftEl.width(),
					fixLeftColumnHtml = "<table class='js-fix-head-freeze"+onlyCls+" fix-head-freeze-left fix-head-freeze' style='width: "+freezeLeftWidth+"px; z-index: 1;'><thead>"+leftColumnHead+"</thead></table>";
				leftEl.before(fixLeftColumnHtml);
			}
			var setWidth = function () {
				ES.each(freezeColumnItems,function (_,v) {
					var el = ES.get(v),
						...
						freezeColumn = "";
					if(alignment.toUpperCase() == "LEFT"){
						freezeColumn = ES.get(".js-column-freeze-left"+onlyCls).find("tbody tr");
					}else if(alignment.toUpperCase() == "RIGHT"){
						freezeColumn = ES.get(".js-column-freeze-right"+onlyCls).find("tbody tr");
					}
					ES.each(freezeColumn,function (k,vl) {
						ES.get(vl).find("td").eq(_).css({
							width: width + "px"
						})
					})
				});
				var leftW = ES.get(".js-column-freeze-left"+onlyCls).outerWidth(),
					rightL = ES.get(".js-column-freeze-right"+onlyCls).outerWidth();
				ES.get(".js-fix-head-freeze"+onlyCls+".fix-head-freeze-left").width(leftW);
				ES.get(".js-fix-head-freeze"+onlyCls+".fix-head-freeze-right").width(rightL);
			}
			ES.get(window).resize(function () {
				setWidth();
			});
		...
		ES.get(document).on("click",function (e) {
			var target = ES.get(e.target);
			if(isAssistBar){
				var isHideAssist = true;
				if(target.hasClass("js-show-assist")){
					isHideAssist = false && isHideAssist;
				}else if(target.parent("span").hasClass("js-show-assist")){
					isHideAssist = false && isHideAssist;
				}else if(target.hasClass("js-assist-bar")){
					isHideAssist = false && isHideAssist;
				}else if(target.parents("div.js-assist-bar").length > 0){
					isHideAssist = false && isHideAssist;
				}
				if(isHideAssist){
					$("div.js-assist-bar").css({
						visibility: "hidden"
					})
					$("span.js-show-assist").removeClass("active");
				}
			}
		});
		if(isFreezeHead){
